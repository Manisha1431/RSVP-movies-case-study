...
-- INFERENCES: Names table has the highest number of rows i.e., 25735 while director_mapping table has the least number of rows i.e., 3867
...
-- INFERENCES: In total, 4 columns have null values and among them worlwide_gross_income has the highest number of null values i.e., 3724
...
-- INFERENCES: The number of movies released each year has declined year by year over the past 3 years.
...
-- INFERENCES: Majority of the movies seem to be releassed in the month of March and the count is least in December
...
-- INFERENCES: USA and India have produced 1007 movies in the year 2019 together.
...
-- INFERENCES: There are 13 distinct movie genres
...
-- INFERENCES: Drama genre had 4285 number of movies produced overall .
...
-- INFERENCES: 3289 movies belong to only one genre.
...
-- INFERENCES: Action genre has the highest average duration i.e., 112.88 mins and horror genre having the least i.e., 92.72 mins
...
-- INFERENCES: The rank of thriller genre is 3 in movie count
...
-- INFERENCES: Both 'Kirket' and 'Love in Kilnerry' has average rating of 10,0 which is the maximum possible rating 
...
-- INFERENCES: Around 94 movies have a median_rating of 1 and around 346 movies have a median_rating of 10
...
-- INFERENCES: Drama genre has the highest value i.e., 16 and family genre has the least value i.e., 1 for this category
...
-- INFERENCES: 'The Brighton Miracle' has the highest avg_rating for this category
...
-- INFERENCES: Height has highest number of null values
...
SELECT director_name,movie_count from 
(
	SELECT 
		name as director_name, 
		count(g.movie_id) as movie_count, 
		RANK() OVER(ORDER BY COUNT(g.movie_id) DESC) as rank_of_directors
	FROM names AS n 
		INNER JOIN director_mapping AS d ON n.id=d.name_id 
		INNER JOIN genre AS g ON d.movie_id=g.movie_id 
		INNER JOIN ratings r ON r.movie_id= g.movie_id, top_genre
...
ON m.id=g.movie_id
INNER JOIN ratings r
USING(movie_id)                  /*   on g.movie_id=r.movie_id  */
